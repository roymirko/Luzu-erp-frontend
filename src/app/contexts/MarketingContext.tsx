...
import { MARKETING_DEFAULTS } from '../utils/implementacionConstants';
...
interface MarketingContextType {
  gastos: Gasto[];
  formulariosAgrupados: FormularioMarketingAgrupado[];
  loading: boolean;
  addGasto: (input: CreateGastoInput) => Promise<boolean>;
  addMultipleGastos: (input: CreateMultipleGastosMarketingInput) => Promise<{ success: boolean; error?: string }>;
...
export function MarketingProvider({ children }: { children: ReactNode }) {
  ...
  const fetchGastos = useCallback(async () => {
    ...
    try {
      ...
      if (result.error) { console.error('[MarketingContext] Error:', result.error); return; }
      setGastos(result.data);
    } catch (err) { console.error('[MarketingContext] Error:', err); }
  ...
  const addMultipleGastos = async (input: CreateMultipleGastosMarketingInput): Promise<{ success: boolean; error?: string }> => {
    const ctxResult = await ctxRepo.create({
      ...
      unidad_negocio: MARKETING_DEFAULTS.unidadNegocio,
      categoria_negocio: MARKETING_DEFAULTS.categoriaNegocio,
      mes_venta: null,
      mes_inicio: null,
      programa: null,
      ejecutivo: null,
      rubro: MARKETING_DEFAULTS.rubro,
  ...
  const formulariosAgrupados = useMemo((): FormularioMarketingAgrupado[] => {
    ...
    const result: FormularioMarketingAgrupado[] = [];
    for (const [formularioId, formularioGastos] of groupedMap) {
      ...
      result.push({
        ...
        subrubro: first.ctxSubRubro || first.subRubro || '',
...
export function useMarketing() {
  ...
  if (context === undefined) throw new Error('useMarketing must be used within a MarketingProvider');
  return context;
}
