...
import { useMarketing } from '@/app/contexts/MarketingContext';
...
export function MarketingForm({ gastoId, existingFormulario, onCancel, onSave }: MarketingFormProps) {
  ...
  const { gastos: contextGastos, loading: contextLoading, addMultipleGastos, addGastoToFormulario, updateGasto, deleteGasto } = useMarketing();
  ...
  // Section 1: Cargar Datos (fixed values + user picks)
  ...
  useEffect(() => {
    ...
    if (contextLoading) { setLoadingData(true); return; }

    const existingGasto = contextGastos.find(g => g.id === gastoId);
    if (existingGasto) {
      setSubrubro(existingGasto.ctxSubRubro || existingGasto.subRubro || '');
      setNombreCampana(existingGasto.ctxNombreCampana || existingGasto.nombreCampana || '');
  ...
  const removeGastoItem = (id: string) => {
    if (gastos.length === 1) { toast.error('Debe haber al menos un gasto'); return; }
  ...
  const handleSaveGasto = async (gasto: GastoItem, index: number): Promise<boolean> => {
    ...
    try {
      if (isExisting) {
        ...
        if (!existingGasto?.formularioId) { toast.error('Debe guardar el formulario primero'); return false; }
        ...
        if (result) {
          ...
          toast.success(`Gasto #${index + 1} creado`); return true;
        }
        toast.error(`Error al crear gasto #${index + 1}`); return false;
      }
    } catch (err) {
      console.error('[MarketingForm] Error guardando gasto:', err);
      toast.error(`Error inesperado al guardar gasto #${index + 1}`); return false;
    } finally {
      setSavingGastos(prev => { const next = new Set(prev); next.delete(gasto.id); return next; });
    }
  };

  const hasErrors = (formErrors: FormErrors): boolean => Object.keys(formErrors).length > 0;
  ...
  const handleGuardar = async () => {
    ...
    if (hasErrors(validationErrors)) { toast.error('Complete todos los campos requeridos'); return; }
    const gastosError = validateGastos();
    if (gastosError) { toast.error(gastosError); return; }
    ...
    try {
      ...
      if (isEditing && gastoId) {
        ...
        if (success) toast.success(`${gastos.length} gasto(s) creado(s)`);
  ...
  const { label: labelClass, disabledSelect: disabledSelectClass, selectTrigger: selectTriggerClass, input: inputClass } = formStyles(isDark);
      ...
      <div className="max-w-5xl mx-auto px-6 sm:px-8 lg:px-12">
        <div className="space-y-6 sm:space-y-8">
          <FormHeader
            ...
            subtitle={isEditing
              ? 'Edite la información del formulario de Marketing'
              : 'Complete la información del nuevo formulario de Marketing'}
          ...
          <Card className={isDark ? 'bg-[#141414] border-gray-800' : 'bg-white border-[#e5e7eb]'}>
            ...
            <CardContent>
              <div className={cn('p-4 rounded-lg border w-[163px]', isDark ? 'bg-[#1e1e1e] border-gray-800' : 'bg-[#f3f5ff] border-[#e5e7eb]')}>
                <p className={cn('text-xs text-center mb-1', isDark ? 'text-gray-400' : 'text-[#4a5565]')}>Total del gasto</p>
                <p className={cn('text-lg font-bold text-center', isDark ? 'text-white' : 'text-[#101828]')}>{formatCurrency(totalGasto)}</p>
              </div>
            </CardContent>
          </Card>
