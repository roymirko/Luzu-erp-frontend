...
import { useProductora } from '../../contexts/ProductoraContext';
...
import { PROGRAMAS_PRODUCTORA_OPTIONS } from '../../utils/implementacionConstants';

const ITEMS_PER_PAGE = 10;

const getProgramaLabel = (value: string | undefined): string => {
  if (!value) return '-';
  const option = PROGRAMAS_PRODUCTORA_OPTIONS.find(o => o.value === value);
  return option?.label || value;
};

type FilterMode = 'programa' | 'campana';

interface TablaProductoraProps {
...
const PROGRAMA_COLUMNS = [
  ...
  'Empresa', 'Unidad de negocio', 'Rubro', 'Subrubro', 'Campaña', 'Proveedor', 'Neto', 'Acciones'
];

const CAMPANA_COLUMNS = [
  'Estado', 'Campaña', 'Fecha de registro', 'Responsable', 'Factura emitida a', 'Empresa',
  'Unidad de negocio', 'Rubro', 'Subrubro', 'Proveedor', 'Neto total', 'Acciones'
];
...
export function TablaProductora({ onOpen, onNew }: TablaProductoraProps) {
  ...
  const { gastos, formulariosAgrupados, loading } = useProductora();
  ...
  const filteredGastos = useMemo(() => {
    ...
    return gastos.filter((g) =>
      ...
      g.formularioRubro?.toLowerCase().includes(s) ||
      g.formularioSubRubro?.toLowerCase().includes(s) ||
  ...
  const filteredFormularios = useMemo(() => {
    ...
    return formulariosAgrupados.filter((f) =>
      ...
      f.rubro?.toLowerCase().includes(s) ||
      ...
      <div className="space-y-6">
        {onNew && (
          <ActionCard
            ...
            description="Crear gasto de productora"
            icon={Plus}
            onClick={onNew}
          />
        )}
          ...
          {currentRows.length === 0 ? (
            ...
            (currentRows as typeof gastos).map((gasto) => (
              <DataTableRow key={gasto.id} onClick={() => handleRowClick(gasto)}>
                ...
                <DataTableCell>{gasto.formularioRubro || '-'}</DataTableCell>
                <DataTableCell>{gasto.formularioSubRubro || '-'}</DataTableCell>
            ...
            (currentRows as typeof formulariosAgrupados).map((formulario) => (
              <DataTableRow key={formulario.id} onClick={() => handleRowClick(formulario)}>
                ...
                <DataTableCell>{formulario.rubro || '-'}</DataTableCell>
